...
pub struct ProcedureEncoder<'p, 'v: 'p, 'tcx: 'v> {
    ...
    mir: &'p mir::Body<'tcx>,
    cfg_method: polymorphic_vir::CfgMethod,
    locals: LocalVariableManager<'tcx>,
    loop_encoder: LoopEncoder<'p, 'tcx>,
    auxiliary_local_vars: HashMap<String, polymorphic_vir::Type>,
    ...
    cfg_block_has_been_executed: HashMap<mir::BasicBlock, polymorphic_vir::LocalVar>,
    /// Magic wand generated by a call at a given location with a label used in post.
    magic_wand_at_location: HashMap<mir::Location, (String, polymorphic_vir::Expr, polymorphic_vir::Expr)>,
    ...
    array_magic_wand_at: HashMap<mir::Location, (polymorphic_vir::Expr, polymorphic_vir::Expr, polymorphic_vir::Expr)>,
    ...
    procedure_contracts:
        HashMap<mir::Location, (ProcedureContract<'tcx>, HashMap<polymorphic_vir::Expr, polymorphic_vir::Expr>)>,
    ...
    pure_var_for_preserving_value_map: HashMap<BasicBlockIndex, HashMap<polymorphic_vir::Expr, polymorphic_vir::LocalVar>>,
    ...
    old_to_ghost_var: HashMap<polymorphic_vir::Expr, polymorphic_vir::Expr>,
    /// Ghost variables used inside package statements.
    old_ghost_vars: HashMap<String, polymorphic_vir::Type>,
...
impl<'p, 'v: 'p, 'tcx: 'v> ProcedureEncoder<'p, 'v, 'tcx> {
    pub fn new(
        ...
        let cfg_method = polymorphic_vir::CfgMethod::new(
            // method name
    ...
    pub fn encode(mut self) -> SpannedEncodingResult<polymorphic_vir::CfgMethod> {
    ...
    //     //trace!("def_id of impl: {:?}", &impl_def_id);
        ...
        let start_cfg_block = self.cfg_method.add_block(
            "start",
            vec![
                polymorphic_vir::Stmt::comment("========== start =========="),
        ...
        let return_cfg_block = self.cfg_method.add_block(
            "return",
            vec![
                polymorphic_vir::Stmt::comment(format!("========== return ==========")),
                polymorphic_vir::Stmt::comment("Target of any 'return' statement."),
            ],
        );
        self.cfg_method
        ...
        for bbi in self.procedure.get_reachable_nonspec_cfg_blocks() {
            let executed_flag_var = self.cfg_method.add_fresh_local_var(polymorphic_vir::Type::Bool);
            let bb_pos = self
                .mir_encoder
                .encode_expr_pos(self.mir_encoder.get_span_of_basic_block(bbi));
            self.cfg_method.add_stmt(
                start_cfg_block,
                polymorphic_vir::Stmt::Assign( polymorphic_vir::Assign {
                    target: polymorphic_vir::Expr::local(executed_flag_var.clone()).set_pos(bb_pos),
                    source: false.into(),
                    kind: polymorphic_vir::AssignKind::Copy,
                }),
            );
    ...
    fn encode_loop(
        ...
        let start_block = self.cfg_method.add_block(
            ...
            vec![polymorphic_vir::Stmt::comment(format!(
                "========== {}_start ==========",
                loop_label_prefix
            ))],
        );
        ...
        self.cfg_method
            .set_successor(inv_pre_block, polymorphic_vir::Successor::Goto(inv_post_block));
        {
            let stmts =
        ...
        self.cfg_method.add_stmt(
            end_body_block,
            polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {expr: false.into()} ),
        );
        ...
        self.cfg_method
            .set_successor(start_block, polymorphic_vir::Successor::Goto(following_block));
        ...
        self.cfg_method
            .set_successor(inv_post_block, polymorphic_vir::Successor::Goto(following_block));
        ...
        self.cfg_method
            .set_successor(end_body_block, polymorphic_vir::Successor::Return);
        ...
        for var in vars {
            let builtin_method = match var.typ {
                polymorphic_vir::Type::Int => BuiltinMethodKind::HavocInt,
                polymorphic_vir::Type::Bool => BuiltinMethodKind::HavocBool,
                polymorphic_vir::Type::TypedRef(_) => BuiltinMethodKind::HavocRef,
                polymorphic_vir::Type::TypeVar(_) => BuiltinMethodKind::HavocRef,
                polymorphic_vir::Type::Domain(_) => BuiltinMethodKind::HavocRef,
                polymorphic_vir::Type::Snapshot(_) => BuiltinMethodKind::HavocRef,
                polymorphic_vir::Type::Seq(_) => BuiltinMethodKind::HavocRef,
            };
            let stmt = polymorphic_vir::Stmt::MethodCall( polymorphic_vir::MethodCall {
    ...
    fn encode_block(
        ...
        if self.loop_encoder.is_loop_head(bbi) {
            self.cfg_method.add_stmt(
                curr_block,
                polymorphic_vir::Stmt::comment("This is a loop head"),
            );
        }
    ...
    fn encode_execution_flag(
        ...
        self.cfg_method.add_stmt(
            cfg_block,
            polymorphic_vir::Stmt::Assign( polymorphic_vir::Assign {
                target: polymorphic_vir::Expr::local(executed_flag_var).set_pos(pos),
                source: true.into(),
                kind: polymorphic_vir::AssignKind::Copy,
            }),
        );
    ...
    fn encode_statement_at(
    ...
    ) -> SpannedEncodingResult<(Vec<polymorphic_vir::Stmt>, Option<MirSuccessor>)> {
        ...
        for (place, variant_idx) in self.mir_encoder.get_downcasts_at_location(location).into_iter() {
            ...
            downcast_stmts.push(polymorphic_vir::Stmt::Downcast( polymorphic_vir::Downcast {base: encoded_place, field: variant_field} ));
        }
        ...
        match stmts_succ_res {
            ...
            Err(err) => {
                ...
                stmts.extend(vec![
                    polymorphic_vir::Stmt::comment(head_stmt),
                    polymorphic_vir::Stmt::comment(
                        format!("Unsupported feature: {}", unsupported_msg)
                    ),
                    polymorphic_vir::Stmt::Assert( polymorphic_vir::Assert {
                        expr: false.into(),
                        position: pos,
                    })
                ]);
    ...
    fn encode_statement(
        ...
        let mut stmts = vec![polymorphic_vir::Stmt::comment(format!("[mir] {:?}", stmt))];
        let span = self.mir_encoder.get_span_of_location(location);
    ...
    fn set_stmts_default_pos(&self, stmts: Vec<polymorphic_vir::Stmt>, default_pos_span: Span) -> Vec<polymorphic_vir::Stmt> {
        let expr_pos = self
            .encoder
            .error_manager()
    ...
    fn encode_assign(
        &mut self,
        encoded_lhs: polymorphic_vir::Expr,
        rhs: &mir::Rvalue<'tcx>,
        ty: ty::Ty<'tcx>,
        location: mir::Location,
    ) -> SpannedEncodingResult<Vec<polymorphic_vir::Stmt>> {
    ...
    fn encode_loan_places(&mut self, loan_places: &LoanPlaces<'tcx>) -> EncodingResult<(polymorphic_vir::Expr, Option<polymorphic_vir::Expr>, bool, Vec<polymorphic_vir::Stmt>)> {
        ...
        if let mir::Rvalue::Ref(_, mir::BorrowKind::Mut { .. }, ref rhs_place) = loan_places.source {
            ...
            if let PlaceEncoding::ArrayAccess { .. } = rhs_place_encoding {
                ...
                stmts.push(
                    polymorphic_vir::Stmt::label(new_lhs_label.clone())
                );
                ...
                let expiring_base_pred = self.mir_encoder.encode_place_predicate_permission(expiring_base.clone(), polymorphic_vir::PermAmount::Write).unwrap();
                stmts.push(vir!{ exhale [expiring_base_pred] });

                let arr_pred = polymorphic_vir::Expr::pred_permission(regained_array.clone(), polymorphic_vir::PermAmount::Write).unwrap();
        ...
        let mut encode = |rhs_place, stmts: &mut Vec<polymorphic_vir::Stmt>, array_encode_kind| -> EncodingResult<_> {
    ...
    fn encode_transfer_permissions(
        &mut self,
        lhs: polymorphic_vir::Expr,
        rhs: polymorphic_vir::Expr,
        location: mir::Location,
        is_in_package_stmt: bool,
    ) -> Vec<polymorphic_vir::Stmt> {
        let mut stmts = if let Some(var) = self.old_to_ghost_var.get(&rhs) {
            vec![polymorphic_vir::Stmt::Assign( polymorphic_vir::Assign {
                target: var.clone(),
                source: lhs.clone(),
                kind: polymorphic_vir::AssignKind::Move,
            })]
        } else {
            vec![polymorphic_vir::Stmt::TransferPerm( polymorphic_vir::TransferPerm {
                left: lhs.clone(),
                right: rhs.clone(),
                unchecked: false,
            })]
        ...
        if self.check_foldunfold_state && !is_in_package_stmt {
            ...
            stmts.push(polymorphic_vir::Stmt::Assert( polymorphic_vir::Assert {
                expr: polymorphic_vir::Expr::eq_cmp(lhs.clone().into(), rhs.into()),
                position: pos,
            }));
        }
    ...
    fn encode_obtain(&mut self, expr: polymorphic_vir::Expr, pos: polymorphic_vir::Position) -> Vec<polymorphic_vir::Stmt> {
        let mut stmts = vec![];

        stmts.push(polymorphic_vir::Stmt::Obtain( polymorphic_vir::Obtain {
            expr: expr.clone(),
            position: pos,
        }));
    ...
    fn construct_vir_reborrowing_dag(
        ...
        is_in_package_stmt: bool,
    ) -> SpannedEncodingResult<polymorphic_vir::borrows::DAG> {
        let mir_dag = self
            .polonius_info()
        ...
        let mut builder = polymorphic_vir::borrows::DAGBuilder::new();
        for node in mir_dag.iter() {
            let node = match node.kind {
                ...
                ReborrowingKind::ArgumentMove { loan } => {
                    ...
                    polymorphic_vir::borrows::Node::new(
                        guard,
    ...
    fn construct_location_guard(&self, location: mir::Location) -> polymorphic_vir::Expr {
        let bbi = &location.block;
        let executed_flag_var = self.cfg_block_has_been_executed[bbi].clone();
        polymorphic_vir::Expr::local(executed_flag_var).into()
    }
    ...
    fn construct_vir_reborrowing_node_for_assignment(
        ...
        end_location: Option<mir::Location>,
        is_in_package_stmt: bool,
    ) -> SpannedEncodingResult<polymorphic_vir::borrows::Node> {
        let span = self.mir_encoder.get_span_of_location(location);
        ...
        Ok(polymorphic_vir::borrows::Node::new(
            guard,
            node.loan.index().into(),
            convert_loans_to_borrows(&node.reborrowing_loans),
            convert_loans_to_borrows(&node.reborrowed_loans),
    ...
    fn construct_vir_reborrowing_node_for_call(
        ...
        let mut stmts: Vec<polymorphic_vir::Stmt> = Vec::new();
        let span = self.mir_encoder.get_span_of_location(location);
        ...
        let replace_fake_exprs = |mut expr: polymorphic_vir::Expr| -> polymorphic_vir::Expr {
        ...
        let statement = polymorphic_vir::Stmt::apply_magic_wand(lhs, rhs, loan.index().into(), pos);
    ...
    fn encode_expiration_of_loans(
        ...
        if loans.len() > 0 {
            ...
            stmts.push(polymorphic_vir::Stmt::ExpireBorrows( polymorphic_vir::ExpireBorrows {
    ...
    fn encode_terminator(
        ...
        location: mir::Location,
    ) -> SpannedEncodingResult<(Vec<polymorphic_vir::Stmt>, MirSuccessor)> {
        debug!(
            "Encode terminator '{:?}', span: {:?}",
            term.kind, term.source_info.span
        );
        let mut stmts: Vec<polymorphic_vir::Stmt> = vec![polymorphic_vir::Stmt::comment(format!("[mir] {:?}", term.kind))];
        let span = self.mir_encoder.get_span_of_location(location);
        ...
        let result = match term.kind {
            ...
            TerminatorKind::SwitchInt {
                ...
                let mut cfg_targets: Vec<(polymorphic_vir::Expr, BasicBlockIndex)> = vec![];

                // Use a local variable for the discriminant (see issue #57)
                let discr_var = match switch_ty.kind() {
                    ty::TyKind::Bool => {
                        self.cfg_method.add_fresh_local_var(polymorphic_vir::Type::Bool)
                    }

                    ty::TyKind::Int(_)
                    | ty::TyKind::Uint(_)
                    | ty::TyKind::Char => {
                        self.cfg_method.add_fresh_local_var(polymorphic_vir::Type::Int)
                    }
                ...
                for (value, target) in targets.iter() {
                    ...
                    let viper_guard = match switch_ty.kind() {
                        ty::TyKind::Bool => {
                            if value == 0 {
                                // If discr is 0 (false)
                                polymorphic_vir::Expr::not(discr_var.clone().into())
                            } else {
                        ...
                        ty::TyKind::Int(_)
                        | ty::TyKind::Uint(_)
                        | ty::TyKind::Char => polymorphic_vir::Expr::eq_cmp(
                            discr_var.clone().into(),
                        ...
                        "std::boxed::Box::<T>::new" => {
                            ...
                            stmts.extend(
                                self.prepare_assign_target(
                                    ...
                                    ref_field,
                                    location,
                                    polymorphic_vir::AssignKind::Move,
                                    false
                                )?
                            );
                        ...
                        => {
                            ...
                            stmts.extend(
                                self.encode_cmp_function_call(
                                    ...
                                    args,
                                    destination,
                                    polymorphic_vir::BinOpKind::EqCmp,
                                )?
                            );
                        }
                        ...
                        => {
                            ...
                            stmts.extend(
                                self.encode_cmp_function_call(
                                    ...
                                    args,
                                    destination,
                                    polymorphic_vir::BinOpKind::NeCmp,
                                )?
                            );
                        }
            ...
            TerminatorKind::Assert {
                ...
                let cond_var = self.cfg_method.add_fresh_local_var(polymorphic_vir::Type::Bool);
                stmts.push(polymorphic_vir::Stmt::Assign( polymorphic_vir::Assign {
                ...
                let viper_guard = if expected {
                    ...
                    polymorphic_vir::Expr::not(cond_var.into())
                };
                ...
                stmts.push(polymorphic_vir::Stmt::comment(format!("Rust assertion: {}", assert_msg)));
                if self.check_panics {
                    stmts.push(polymorphic_vir::Stmt::Assert( polymorphic_vir::Assert {
                        expr: viper_guard,
                    ...
                    stmts.push(polymorphic_vir::Stmt::comment("This assertion will not be checked"));
                    stmts.push(polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {
                        expr: viper_guard,
                    }));
                };
    ...
    fn encode_slice_len_call(
        ...
        stmts.push(polymorphic_vir::Stmt::label(label.clone()));
    ...
    fn encode_cmp_function_call(
        ...
        args: &[mir::Operand<'tcx>],
        destination: &Option<(mir::Place<'tcx>, BasicBlockIndex)>,
        bin_op: polymorphic_vir::BinOpKind,
    ) -> SpannedEncodingResult<Vec<polymorphic_vir::Stmt>> {
        ...
        if self.encoder.supports_snapshot_equality(&arg_ty).with_span(call_site_span)? {
            ...
            let expr = match bin_op {
                polymorphic_vir::BinOpKind::EqCmp => polymorphic_vir::Expr::eq_cmp(
                    polymorphic_vir::Expr::snap_app(lhs),
                    polymorphic_vir::Expr::snap_app(rhs),
                ),
                polymorphic_vir::BinOpKind::NeCmp => polymorphic_vir::Expr::ne_cmp(
                    polymorphic_vir::Expr::snap_app(lhs),
                    polymorphic_vir::Expr::snap_app(rhs),
                ),
                _ => unreachable!()
            ...
            let (target_value, mut stmts) = self.encode_pure_function_call_lhs_value(destination)
                .with_span(call_site_span)?;
            let inhaled_expr = polymorphic_vir::Expr::eq_cmp(target_value.into(), expr);
    ...
    fn encode_edge_block(
        ...
        if force_block || !stmts.is_empty() {
            ...
            let edge_block = self.cfg_method.add_block(
                &edge_label,
                vec![
                    polymorphic_vir::Stmt::comment(format!("========== {} ==========", edge_label)),
                    polymorphic_vir::Stmt::comment(format!("MIR edge {:?} --> {:?}", source, destination)),
                ],
            );
            if !stmts.is_empty() {
                self.cfg_method
                    .add_stmt(edge_block, polymorphic_vir::Stmt::comment("Expire borrows"));
    ...
    fn encode_impure_function_call(
        ...
        let mut operands: Vec<(&mir::Operand<'tcx>, Local, ty::Ty<'tcx>, Option<polymorphic_vir::Expr>)> = vec![];
        let mut encoded_operands = mir_args.iter()
            .map(|arg| self.mir_encoder.encode_operand_place(&arg))
            .collect::<Result<Vec<Option<polymorphic_vir::Expr>>, _>>()
            .with_span(call_site_span)?;
        ...
        for (mir_arg, arg, arg_ty, encoded_operand) in operands {
            ...
            let arg_place = polymorphic_vir::Expr::local(encoded_local);
        ...
        let (target_local, encoded_target) = {
            match destination.as_ref() {
                Some((ref target_place, _)) => {
                    ...
                    fake_exprs.insert(
                        polymorphic_vir::Expr::local(self.encode_prusti_local(target_local)),
                ...
                None => {
                    ...
                    stmts_after.push(polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {
                        expr: false.into()
                    }));
                ...
                expr = expr
                    .fold_expr(|orig_expr| {
                        ...
                        match orig_expr {
                            ...
                            polymorphic_vir::Expr::PredicateAccessPredicate( polymorphic_vir::PredicateAccessPredicate {ref argument, ..} ) => {
        ...
        stmts.push(polymorphic_vir::Stmt::label(pre_label.clone()));
        ...
        stmts.push(polymorphic_vir::Stmt::Exhale( polymorphic_vir::Exhale {
        ...
        for perm in pre_mandatory_perms {
            ...
            stmts.push(polymorphic_vir::Stmt::TransferPerm( polymorphic_vir::TransferPerm {
                left: from_place,
                right: to_place,
                unchecked: true
            }));
        ...
        stmts.push(polymorphic_vir::Stmt::label(post_label.clone()));
    ...
    fn encode_specified_pure_function_call(
        ...
        let func_call = polymorphic_vir::Expr::func_app(
            function_name.clone(),
            arg_exprs,
            formal_args,
            return_type.clone(),
        ...
        let inhaled_expr = if return_type.is_domain() {
            ...
            polymorphic_vir::Expr::eq_cmp(
                polymorphic_vir::Expr::snap_app(target_place),
                func_call,
            )
        } else {
            polymorphic_vir::Expr::eq_cmp(target_value.into(), func_call)
        };
    ...
    fn encode_pure_function_call_lhs_place(
        &mut self,
        destination: &Option<(mir::Place<'tcx>, BasicBlockIndex)>,
    ) -> (polymorphic_vir::Expr, Vec<polymorphic_vir::Stmt>) {
        match destination.as_ref() {
            // will panic if attempting to encode unsupported type
            Some((ref dst, _)) => {
    ...
    fn encode_pure_function_call_site(
        ...
        location: mir::Location,
        destination: &Option<(mir::Place<'tcx>, BasicBlockIndex)>,
        call_result: polymorphic_vir::Expr,
    ) -> (Vec<polymorphic_vir::Stmt>, String) {
        let mut stmts = vec![];
        ...
        let type_predicate = self
            .mir_encoder
            .encode_place_predicate_permission(target_place.clone(), polymorphic_vir::PermAmount::Write)
            .unwrap();
        ...
        // Initialize the lhs
        stmts.push(
            polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {
                expr: call_result,
            })
        );
    ...
    fn encode_transfer_args_permissions(
        ...
        location: mir::Location,
        args: &[mir::Operand<'tcx>],
        stmts: &mut Vec<polymorphic_vir::Stmt>,
        label: String,
        is_in_package_stmt: bool,
    ) -> SpannedEncodingResult<()> {
        let span = self.mir_encoder.get_span_of_location(location);
        ...
        Ok(match self.locals.get_type(local).kind() {
            ...
            | ty::TyKind::Ref(_, ref ty, mutability) => {
                ...
                let perm_amount = match mutability {
                    ...
                    Mutability::Not => polymorphic_vir::PermAmount::Read,
                };
                polymorphic_vir::Expr::and(
                    polymorphic_vir::Expr::acc_permission(place.clone(), polymorphic_vir::PermAmount::Write),
                    polymorphic_vir::Expr::pred_permission(place, perm_amount).unwrap(),
                )
            }
            _ => self
                .mir_encoder
    ...
    fn encode_precondition_expr(
        ...
        for local in &contract.args {
            let mut add = |access: polymorphic_vir::Expr| {
                if is_blocked(maybe_blocked_paths, *local)
                    && access.get_perm_amount() == polymorphic_vir::PermAmount::Read
                {
        ...
        let mut invs_spec: Vec<polymorphic_vir::Expr> = vec![];
    ...
    fn encode_preconditions(
        ...
        self.cfg_method
            .add_stmt(start_cfg_block, polymorphic_vir::Stmt::comment("Preconditions:"));
        ...
        self.cfg_method.add_stmt(
            start_cfg_block,
            polymorphic_vir::Stmt::label(PRECONDITION_LABEL),
        );
        Ok(())
    }
    ...
    fn encode_postcondition_magic_wand(
        ...
        pre_label: &str,
        post_label: &str,
    ) -> EncodingResult<Option<(polymorphic_vir::Expr, polymorphic_vir::Expr)>> {
        ...
        let encoded_args: Vec<polymorphic_vir::Expr> = contract
            ...
            .map(|local| self.encode_prusti_local(*local).into())
            .collect();
        let encoded_return: polymorphic_vir::Expr = self.encode_prusti_local(contract.returned_value).into();
        ...
        if !borrow_infos.is_empty() {
            ...
            let encode_place_perm = |place, mutability, label| -> _ {
                ...
                let vir_access =
                    polymorphic_vir::Expr::pred_permission(place_expr.clone().old(label), perm_amount).unwrap();
                self
                    .encoder
                    .encode_invariant_func_app(place_ty, place_expr.old(label))
                    // TODO: Use a better span
                    .with_span(self.mir.span)
                    .map(|inv| polymorphic_vir::Expr::and(vir_access, inv))
            };
            ...
            if let Some(typed::Pledge { reference, lhs: body_lhs, rhs: body_rhs}) = pledges.first() {
                ...
                let old_expr = polymorphic_vir::Expr::labelled_old(post_label, original_expr.clone());
                // TODO ??
    ...
    fn wrap_arguments_into_old(
        &self,
        mut assertion: polymorphic_vir::Expr,
        pre_label: &str,
        contract: &ProcedureContract<'tcx>,
        encoded_args: &[polymorphic_vir::Expr],
    ) -> SpannedEncodingResult<polymorphic_vir::Expr> {
        for (encoded_arg, &arg) in encoded_args.iter().zip(&contract.args) {
            ...
            if is_reference(ty) {
                ...
                let (encoded_deref, ..) = self
                    ...
                    .with_span(arg_span)?;
                let original_expr = encoded_deref;
                let old_expr = polymorphic_vir::Expr::labelled_old(pre_label, original_expr.clone());
                ...
                assertion = assertion.fold_places(|place| {
                    let base: polymorphic_vir::Expr = place.get_base().into();
                    if encoded_arg == &base {
                        place.old(pre_label)
                    } else {
                        place
    ...
    fn encode_postcondition_expr(
        ...
        pre_label: &str,
        post_label: &str,
        magic_wand_store_info: Option<(mir::Location, &HashMap<polymorphic_vir::Expr, polymorphic_vir::Expr>)>,
        _diverging: bool,
        loan: Option<facts::Loan>,
        function_end: bool,
    ) -> SpannedEncodingResult<(
        polymorphic_vir::Expr,                   // Returned permissions from types.
        Option<polymorphic_vir::Expr>,           // Permission of the return value.
        polymorphic_vir::Expr,                   // Invariants.
        polymorphic_vir::Expr,                   // Functional specification.
        Vec<polymorphic_vir::Expr>,              // Magic wands.
        Vec<(polymorphic_vir::Expr, polymorphic_vir::Expr)>, // Read permissions that need to be transferred to a new place.
        Option<polymorphic_vir::Expr>, // Specification strengthening, in case of trait method implementation.
    )> {
        ...
        for (place, mutability) in contract.returned_refs.iter() {
            ...
            let mut add_type_spec = |perm_amount| {
                let permissions =
                    polymorphic_vir::Expr::pred_permission(old_place_expr.clone(), perm_amount).unwrap();
                type_spec.push(permissions);
            };
            match mutability {
                Mutability::Not => {
                    if function_end {
                        add_type_spec(polymorphic_vir::PermAmount::Read);
                    }
                    read_transfer.push((place_expr, old_place_expr));
                }
                Mutability::Mut => {
                    add_type_spec(polymorphic_vir::PermAmount::Write);
                    let inv = self
                        .encoder
        ...
        if let Some((mut lhs, mut rhs)) = self.encode_postcondition_magic_wand(
            ...
            magic_wands.push(polymorphic_vir::Expr::magic_wand(lhs, rhs, loan.map(|l| l.index().into())));
        }
    ...
    fn replace_old_places_with_ghost_vars(
        ...
        struct OldReplacer<'a> {
            label: Option<&'a str>,
            old_to_ghost_var: &'a mut HashMap<polymorphic_vir::Expr, polymorphic_vir::Expr>,
            old_ghost_vars: &'a mut HashMap<String, polymorphic_vir::Type>,
            cfg_method: &'a mut polymorphic_vir::CfgMethod,
        }
        impl<'a> polymorphic_vir::ExprFolder for OldReplacer<'a> {
            fn fold_labelled_old(
                ...
                let expr = polymorphic_vir::Expr::LabelledOld( polymorphic_vir::LabelledOld {
                    label: label.clone(),
                    base,
                    position: pos,
                });
                ...
                if self.old_to_ghost_var.contains_key(&expr) {
                    ...
                    let var: polymorphic_vir::Expr = polymorphic_vir::LocalVar::new(name, vir_type).into();
        ...
        polymorphic_vir::ExprFolder::fold(&mut replacer, expr)
    }
    ...
    fn encode_package_end_of_method(
        ...
        if let Some((lhs, rhs)) = self.encode_postcondition_magic_wand(
            ...
            package_stmts.push(polymorphic_vir::Stmt::comment("Fold predicates for &mut args"));
            for arg_index in self.mir.args_iter() {
                ...
                if is_reference(arg_ty) {
                    ...
                    let predicate =
                        polymorphic_vir::Expr::pred_permission(old_deref_place, polymorphic_vir::PermAmount::Write)
                            .unwrap();
            ...
            let vars: Vec<_> = self
                .old_ghost_vars
                .iter()
                .map(|(name, typ)| polymorphic_vir::LocalVar::new(name.clone(), typ.clone()))
                .collect();
            stmts.push(polymorphic_vir::Stmt::package_magic_wand(
                lhs,
                rhs,
                package_stmts,
                post_label.clone(),
    ...
    fn encode_postconditions(
        ...
        self.cfg_method.add_stmt(return_cfg_block, polymorphic_vir::Stmt::comment("Exhale postcondition"));

        let postcondition_label = self.cfg_method.get_fresh_label_name();
        self.cfg_method.add_stmt(return_cfg_block, polymorphic_vir::Stmt::label(postcondition_label.clone()));
        ...
        for (i, &arg) in contract.args.iter().enumerate() {
            ...
            let ty = self.locals.get_type(arg);
            if is_reference(ty) {
                let encoded_arg: polymorphic_vir::Expr = self.encode_prusti_local(arg).into();
                let arg_span = self.mir_encoder.get_local_span(arg.into());
                let (encoded_deref, ..) = self
                    .mir_encoder
                    .encode_deref(encoded_arg.clone(), ty)
        ...
        self.cfg_method.add_stmt(
            return_cfg_block,
            polymorphic_vir::Stmt::comment("Fold the result"),
        );
        let ty = self.locals.get_type(contract.returned_value);
        let encoded_return: polymorphic_vir::Expr = self.encode_prusti_local(contract.returned_value).into();
        ...
        let return_pred = self
            .mir_encoder
            .encode_place_predicate_permission(encoded_return_expr, polymorphic_vir::PermAmount::Write)
            .unwrap();
        let obtain_return_stmt = polymorphic_vir::Stmt::Obtain( polymorphic_vir::Obtain {
            expr: return_pred,
        ...
        self.cfg_method.add_stmt(
            return_cfg_block,
            polymorphic_vir::Stmt::comment("Assert possible strengthening"),
        );
        ...
        self.cfg_method.add_stmt(
            return_cfg_block,
            polymorphic_vir::Stmt::comment("Assert functional specification of postcondition"),
        );
        let func_pos = self
            .encoder
            .error_manager()
        ...
        self.cfg_method.add_stmt(
            return_cfg_block,
            polymorphic_vir::Stmt::comment("Assert type invariants"),
        );
        ...
        self.cfg_method.add_stmt(
            return_cfg_block,
            polymorphic_vir::Stmt::comment("Exhale permissions of postcondition (1/3)"),
        );
        let perm_pos = self
            .encoder
            .error_manager()
        ...
        self.cfg_method.add_stmt(
            return_cfg_block,
            polymorphic_vir::Stmt::Exhale( polymorphic_vir::Exhale {
        ...
        self.cfg_method.add_stmt(
            return_cfg_block,
            polymorphic_vir::Stmt::comment("Exhale permissions of postcondition (2/3)"),
        );
        if let Some(access) = return_type_spec {
            self.cfg_method.add_stmt(
                return_cfg_block,
        ...
        self.cfg_method.add_stmt(
            return_cfg_block,
            polymorphic_vir::Stmt::comment("Exhale permissions of postcondition (3/3)"),
        );
    ...
    fn get_pure_var_for_preserving_value(
        ...
        if let Some(local_var) = loop_map.get(place) {
            ...
            let vir_type = polymorphic_vir::Type::typed_ref("AuxRef");
            self.cfg_method.add_local_var(&name, vir_type.clone());
            self.auxiliary_local_vars
                .insert(name.clone(), vir_type.clone());
            let var = polymorphic_vir::LocalVar::new(name, vir_type);
    ...
    fn construct_value_preserving_equality(
        ...
        polymorphic_vir::Expr::BinOp ( polymorphic_vir::BinOp {
            op_kind: polymorphic_vir::BinOpKind::EqCmp,
    ...
    fn construct_value_preserving_array_equality(
        &mut self,
        loop_head: BasicBlockIndex,
        array_base: polymorphic_vir::Expr,
    ) -> polymorphic_vir::Expr {
        ...
        let snap_array = polymorphic_vir::Expr::snap_app(array_base);
        let old_snap_array = polymorphic_vir::Expr::old(snap_array.clone(), old_label);
    ...
    ) -> EncodingResult<(Vec<polymorphic_vir::Expr>, Vec<polymorphic_vir::Expr>, Vec<polymorphic_vir::Expr>)> {
        trace!(
            ...
            for (kind, mir_place) in tree.get_permissions().into_iter() {
                ...
                match kind {
                    ...
                    PermissionKind::ReadNode => {
                        if is_array_access {
                            ...
                            array_pred_perms.entry(encoded_place)
                                .or_insert(polymorphic_vir::PermAmount::Read);
                        } else {
                            let perm = polymorphic_vir::Expr::acc_permission(encoded_place, polymorphic_vir::PermAmount::Read);
                    ...
                    PermissionKind::WriteNode => {
                        if is_array_access {
                            array_pred_perms.insert(encoded_place, polymorphic_vir::PermAmount::Write);
                        } else {
                            let perm = polymorphic_vir::Expr::acc_permission(encoded_place, polymorphic_vir::PermAmount::Write);
                            permissions.push(perm);
                        }
                    }
                    ...
                    PermissionKind::ReadSubtree | PermissionKind::WriteSubtree => {
                        let perm_amount = match kind {
                            PermissionKind::WriteSubtree => polymorphic_vir::PermAmount::Write,
                            PermissionKind::ReadSubtree => polymorphic_vir::PermAmount::Read,
                            _ => unreachable!(),
                        };
                        let def_init = self
                            .loop_encoder
                        ...
                        match ty.kind() {
                            ...
                            | ty::TyKind::Ref(_, ref ty, mutbl) => {
                                ...
                                // Use unfolded references.
                                let field = self.encoder.encode_dereference_field(ty)?;
                                let field_place = polymorphic_vir::Expr::from(encoded_place).field(field);
                                permissions.push(polymorphic_vir::Expr::acc_permission(
                                    field_place.clone(),
                                    perm_amount,
                                ));
                                if def_init {
                                ...
                                {
                                    permissions.push(
                                        polymorphic_vir::Expr::pred_permission(field_place, perm_amount)
                                            .unwrap(),
                                    );
                                }
                            }
                            _ => {
                                if is_array_access {
                                    array_pred_perms.entry(encoded_place)
                                        .and_modify(|e| if perm_amount == polymorphic_vir::PermAmount::Write { *e = polymorphic_vir::PermAmount::Write; })
                                        .or_insert(perm_amount);
                                } else {
                                    permissions.push(
                                        polymorphic_vir::Expr::pred_permission(encoded_place, perm_amount).unwrap(),
                                    );
                                }
    ...
    fn encode_loop_invariant_specs(
    ...
    ) -> SpannedEncodingResult<(Vec<polymorphic_vir::Expr>, MultiSpan)> {
        ...
        if !specs.is_empty() {
            let encoded_args: Vec<polymorphic_vir::Expr> = self
                .mir
                .args_iter()
    ...
    fn encode_loop_invariant_exhale_stmts(
        ...
        let obtain_predicates = permissions.iter().map(|p| {
            polymorphic_vir::Stmt::Obtain( polymorphic_vir::Obtain {
                expr: p.clone(),
                position: assert_pos,
    ...
    fn encode_generic_place(
        ...
        match place {
            ...
            Place::SubstitutedPlace {
                ...
                struct RootReplacer {
                    new_root: polymorphic_vir::LocalVar,
                }
    ...
    fn encode_array_direct_assign(
        ...
        let array_acc_expr = polymorphic_vir::Expr::predicate_access_predicate(
        ...
        let old = |e| { polymorphic_vir::Expr::labelled_old(&label, e) };

        let idx_val_int = self.encoder.patch_snapshots(polymorphic_vir::Expr::snap_app(index)).with_span(span)?;

        // inhale infos about array contents back
        let i_var: polymorphic_vir::Expr = vir_local!{ i: Int }.into();
        let zero_le_i = vir!{ [polymorphic_vir::Expr::from(0)] <= [ i_var ] };
        let i_lt_len = vir!{ [ i_var ] < [ polymorphic_vir::Expr::from(array_types.array_len) ] };
            ...
            mir::Operand::Move(ref place) => {
                ...
                let encode_stmts = match ty.kind() {
                    ty::TyKind::RawPtr(..) | ty::TyKind::Ref(..) => {
                        ...
                        alloc_stmts.push(polymorphic_vir::Stmt::Assign( polymorphic_vir::Assign {
                ...
                let encode_stmts = match ty.kind() {
                    ...
                    ty::TyKind::Ref(..) => {
                        ...
                        stmts.push(polymorphic_vir::Stmt::Assign( polymorphic_vir::Assign {
                            ...
                            kind: polymorphic_vir::AssignKind::SharedBorrow(loan.index().into()),
                        }));
                        stmts
                    }
    ...
    fn encode_copy_value_assign(
        &mut self,
        encoded_lhs: polymorphic_vir::Expr,
        encoded_rhs: polymorphic_vir::Expr,
        ty: ty::Ty<'tcx>,
        location: mir::Location,
    ) -> SpannedEncodingResult<Vec<polymorphic_vir::Stmt>> {
        let span = self.mir_encoder.get_span_of_location(location);
    ...
    fn encode_assign_checked_binary_op(
            ...
            .is_vir_place_accessible(&encoded_lhs, location)
        {
            ...
            let mut inhale_acc = |place| {
                alloc_stmts.push(polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {
                    expr: polymorphic_vir::Expr::acc_permission(place, polymorphic_vir::PermAmount::Write),
                }));
            };
    ...
    fn encode_assign_ref(
        ...
        let (vir_assign_kind, array_encode_kind) = match mir_borrow_kind {
            mir::BorrowKind::Shared =>
                (polymorphic_vir::AssignKind::SharedBorrow(loan.index().into()), ArrayAccessKind::Shared),
            ...
            mir::BorrowKind::Mut { .. } =>
                (polymorphic_vir::AssignKind::MutableBorrow(loan.index().into()),
    ...
    fn encode_assign_slice(
        ...
        stmts.push(vir!{ inhale [polymorphic_vir::Expr::FieldAccessPredicate( polymorphic_vir::FieldAccessPredicate {
            base: box slice_expr.clone(),
            permission: polymorphic_vir::PermAmount::Write,
            position: polymorphic_vir::Position::default(),
        })]});

        let slice_perm = polymorphic_vir::Expr::PredicateAccessPredicate( polymorphic_vir::PredicateAccessPredicate {
            predicate_type: slice_types.slice_pred_type.clone(),
            argument: box slice_expr.clone(),
            permission: if is_mut { polymorphic_vir::PermAmount::Write } else { polymorphic_vir::PermAmount::Read },
            position: polymorphic_vir::Position::default(),
        });
        ...
        stmts.push(polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {
            expr: vir!{ [slice_len_call] == [polymorphic_vir::Expr::from(array_types.array_len)] }
        }));
        ...
        for idx in 0..array_types.array_len {
            ...
            stmts.push(polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {
                expr: vir!{ [array_lookup_call] == [slice_lookup_call] }
            }));
        }
    ...
    fn encode_assign_sequence_len(
        ...
        match place_ty.kind() {
            ...
            ty::TyKind::Slice(..) => {
                ...
                stmts.push(polymorphic_vir::Stmt::Assert( polymorphic_vir::Assert {
                    expr: polymorphic_vir::Expr::predicate_access_predicate(
    ...
    fn encode_assign_array_repeat_initializer(
        ...
        for i in 0..len {
            let idx = polymorphic_vir::Expr::from(i);
            ...
            stmts.push(polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {
                expr: vir!{ [lookup_pure_call] == [encoded_operand] }
            }));
        }
    ...
    pub fn get_auxiliary_local_var(&mut self, suffix: &str, vir_type: polymorphic_vir::Type) -> polymorphic_vir::LocalVar {
        ...
        polymorphic_vir::LocalVar::new(name, vir_type)
    }

    fn encode_havoc(&mut self, dst: &polymorphic_vir::Expr) -> Vec<polymorphic_vir::Stmt> {
        debug!("Encode havoc {:?}", dst);
        let havoc_ref_method_name = self
            .encoder
            .encode_builtin_method_use(BuiltinMethodKind::HavocRef);
        if let &polymorphic_vir::Expr::Local( polymorphic_vir::Local {variable: ref dst_local_var, ..} ) = dst {
            vec![polymorphic_vir::Stmt::MethodCall( polymorphic_vir::MethodCall {
    ...
    fn encode_havoc_and_allocation(&mut self, dst: &polymorphic_vir::Expr) -> Vec<polymorphic_vir::Stmt> {
        ...
        stmts.push(polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {
            expr: self.mir_encoder
                    .encode_place_predicate_permission(dst.clone(), polymorphic_vir::PermAmount::Write)
                    .unwrap(),
        }));
        stmts
    }
    ...
    fn prepare_assign_target(
        ...
        if !self.init_info.is_vir_place_accessible(&dst, location) {
            ...
            let acc = polymorphic_vir::Expr::acc_permission(dst_field.clone(), polymorphic_vir::PermAmount::Write);
            alloc_stmts.push(polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {
                expr: acc,
            }));
            match vir_assign_kind {
                polymorphic_vir::AssignKind::Copy => {
                    if field.typ.is_typed_ref_or_type_var() {
                        if can_copy_reference {
                            let pred_acc = polymorphic_vir::Expr::predicate_access_predicate(
                                field.typ,
                                dst_field,
                                polymorphic_vir::PermAmount::Read
                            );
                            alloc_stmts.push(polymorphic_vir::Stmt::Inhale(
                                polymorphic_vir::Inhale { expr: pred_acc }
                            ));
                        } else {
                ...
                polymorphic_vir::AssignKind::Ghost => unreachable!(),
            }
    ...
    fn encode_copy_snapshot_value(
        ...
        stmts.push(polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {
            expr: polymorphic_vir::Expr::eq_cmp(
                polymorphic_vir::Expr::snap_app(src),
                polymorphic_vir::Expr::snap_app(dst),
            ),
        }));
        Ok(stmts)
    }
        ...
        match *aggregate {
            ...
            mir::AggregateKind::Adt(adt_def, variant_index, subst, _, _) if !adt_def.is_union() => {
                ...
                if num_variants != 1 {
                    ...
                    let discr_value: polymorphic_vir::Expr = if let SignedInt(ity) = adt_def.repr.discr_type() {
                        let bit_size =
                    ...
                    stmts.push(polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {
                        expr: polymorphic_vir::Expr::eq_cmp(discriminant, discr_value),
                    }));
                    ...
                    if !variant_def.fields.is_empty() {
                        stmts.push(polymorphic_vir::Stmt::Downcast( polymorphic_vir::Downcast {
                            base: dst.clone(),
                            field: variant_field,
                        }));
                    }
    ...
    ) -> EncodingResult<(polymorphic_vir::Expr, Vec<polymorphic_vir::Stmt>, ty::Ty<'tcx>, Option<usize>)> {
    ...
    fn encode_array_lookup_shared(
        ...
        stmts.push(polymorphic_vir::Stmt::Assert( polymorphic_vir::Assert {
            expr: polymorphic_vir::Expr::predicate_access_predicate(array_types.array_pred_type, encoded_base_expr, polymorphic_vir::PermAmount::Read),
            position: polymorphic_vir::Position::default(),
        }));

        stmts.push(polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {
            expr: vir!{ [ lookup_pure_call ] == [ lookup_res_val_field ] }
        }));
        Ok((lookup_res, stmts))
    }
    ...
    fn encode_array_lookup_mut(
        ...
        stmts.push(polymorphic_vir::Stmt::label(&before_label));

        // exhale preconditions
        stmts.push(polymorphic_vir::Stmt::Assert( polymorphic_vir::Assert {
            expr: vir!{ [idx_val_int] < [ polymorphic_vir::Expr::from(array_types.array_len) ] },
            position: polymorphic_vir::Position::default(),
        }));

        // exhale Array$4$i32(self)
        let array_access_pred = polymorphic_vir::Expr::pred_permission(
            encoded_base_expr.clone(),
            polymorphic_vir::PermAmount::Write,
        ).unwrap();
        ...
        let old = |e| { polymorphic_vir::Expr::labelled_old(&before_label, e) };
        let old_lhs = |e| { polymorphic_vir::Expr::labelled_old("lhs", e) };
        ...
        stmts.push(polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {
            expr: vir!{ [ old(lookup_pure_call) ] == [ res_val_field ] }
        }));
    ...
    fn postprocess_place_encoding(
        ...
        Ok(match place_encoding {
            ...
            PlaceEncoding::SliceAccess { base, index, rust_slice_ty, .. } => {
                ...
                stmts.push(polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {
                    expr: vir!{ [lookup_pure_call] == [res_val_field] }
                }));
