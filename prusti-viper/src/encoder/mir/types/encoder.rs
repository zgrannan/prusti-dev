...
use crate::encoder::{
    ...
    errors::{EncodingError, EncodingResult, SpannedEncodingError, SpannedEncodingResult, ErrorCtxt},
    foldunfold,
...
use prusti_common::vir::polymorphic_vir::PermAmount;
use vir_crate::{common::expression::{less_equals, ExpressionIterator}, high::{self as vir, VariableDecl, ty::VariantIndex, visitors::ExpressionFolder}, polymorphic::{EnumVariantIndex, Expr, ExprFolder, Local, LocalVar, ast}};
    ...
    pub fn encode_invariant_def(self, invariant_name: &str) -> EncodingResult<vir::FunctionDecl> {
        ...
        let field_invariants = match self.ty.kind() {
            ty::TyKind::Ref(_, ref ty, _) if should_encode_typ_invariant(ty) => {
                ...
                let elem_loc = Expr::from(self_local_var.clone()).field(elem_field);
                Some(vec![self
                    .encoder
                    .encode_invariant_func_app(ty, elem_loc)?])
            }
            ty::TyKind::Adt(ref adt_def, ref subst) if !adt_def.is_box() => {
                let mut exprs: Vec<Exprression> = vec![];
                let num_variants = adt_def.variants.len();
                let tcx = self.encoder.env().tcx();
                if let Some(spec) = self.encoder.get_struct_specs(adt_def.did) {
                    ...
                    if num_variants == 0 {
                        ...
                        for field in &adt_def.non_enum_variant().fields {
                            ...
                            let elem_loc =
                                Expr::from(self_local_var.clone()).field(elem_field);
                ...
                if num_variants > 1 {
                    ...
                    let discriminant_loc =
                        Expr::from(self_local_var.clone()).field(discriminant_field.clone());
                    adt_def.variants.iter().zip(discriminant_values).for_each(
                        |(variant_def, variant_index)| {
                            let variant_name = &variant_def.ident.as_str();
                            let variant_loc =
                                Expr::from(self_local_var.clone()).variant(variant_name);
                            let variant_predicate = typ.clone().variant(
                                VariantIndex { index: variant_name.to_string() }
                            );
                            let guard =
                                Expr::eq_cmp(discriminant_loc.clone(), variant_index.into());
                            variant_def.fields.iter().for_each(|field| {
                                ...
                                let variant_field =
                                    vir::FieldDecl::new(
                                        variant_name,
                                        self.encoder.encode_type_high(varaint_def)?
                                    );
                                let invariant_expr = Expression::downcast(
                                    invariant_expr,
                                    Expression::local(self_local_var),
                                    variant_field
                                );
                                let implication = Expression::implies(guard.clone(), invariant_expr);
        ...
        let precondition = match self.ty.kind() {
            ty::TyKind::RawPtr(ty::TypeAndMut { ref ty, .. }) | ty::TyKind::Ref(_, ref ty, _) => {
                ...
                let elem_loc = Expr::from(self_local_var.clone()).field(elem_field);
                Expr::and(
                    Expr::acc_permission(elem_loc.clone(), PermAmount::Read),
                    Expr::pred_permission(elem_loc, PermAmount::Read).unwrap(),
                )
            }
            _ => Expr::predicate_access_predicate(
        ...
        let function = vir::FunctionDecl {
            ...
            parameters: vec![VariableDecl::new(self_local_var.name, typ)],
        ...
        // let function = self
        //     .encoder
        //     .patch_snapshots_function(function, &HashMap::new())?;
        ...
        // let function =
        //     foldunfold::add_folding_unfolding_to_function(function, predicates_map).unwrap(); // TODO: generate a stub function in case of error
...
impl ExprFolder for HackyExprFolder {
    fn fold_local(&mut self, Local { position, .. }: Local) -> Expr {
        Expr::local_with_pos(self.saelf.clone(), position)
    }
}
