        ...
        let expr = vir::with_vcx(move |vcx| {
            ...
            let body = match kind {
                ...
                PureKind::Spec => {
                    vcx.body
                        .borrow_mut()
                        .get_spec_body(def_id, substs, caller_def_id)
                }
                PureKind::Pure => {
                    vcx.body
                        .borrow_mut()
                ...
                PureKind::Constant(promoted) => vcx
                    .body
                    .borrow_mut()
                    .get_promoted_constant_body(def_id, promoted),
            };
...
impl<'tcx, 'vir: 'enc, 'enc> Enc<'tcx, 'vir, 'enc> {
    ...
    fn get_ty_for_local(&mut self, local: mir::Local) -> vir::Type<'vir> {
        ...
        if let ty::TyKind::Closure(..) = ty.kind() {
            ...
            self.deps.require_ref::<SnapshotEnc>(ty).unwrap().generic_snapshot.snapshot
        }
    }
    ...
    fn encode_cfg(
        ...
        match &term.kind {
            ...
            mir::TerminatorKind::SwitchInt { discr, targets } => {
                ...
                let discr_ty_out = self.deps.require_local::<SnapshotEnc>(discr_ty).unwrap().generic_snapshot
                    .specifics
                    .expect_primitive();
                ...
                let expr = match func_ty.kind() {
                    &TyKind::FnDef(def_id, arg_tys) => {
                        ...
                        if is_pure {
                            let pure_func = self
                                .deps
                                .require_ref::<MirFunctionEnc>((def_id, arg_tys, self.def_id))
                                .unwrap()
                                .function_ref;
        ...
        match rvalue {
            ...
            mir::Rvalue::Ref(_, kind, place) => {
                let rvalue_snapshot_encoding =
                    self.deps.require_local::<SnapshotEnc>(rvalue_ty).unwrap().generic_snapshot;
            ...
            mir::Rvalue::Aggregate(box kind, fields) => match kind {
                mir::AggregateKind::Closure(..) => {
                    ...
                    let tuple_ref = self
                        .deps
                        .require_local::<ViperTupleEnc>(fields.len())
                        .unwrap();
                    let fields = fields
                        .iter()
            ...
            mir::Rvalue::Discriminant(place) => {
                ...
                let ty = self.deps.require_local::<SnapshotEnc>(place_ty.ty).unwrap().generic_snapshot
                    .specifics;
                match ty
                    .get_enumlike()
                    .filter(|_| place_ty.variant_index.is_none())
                {
                    ...
                    None => {
                        let e_rvalue_ty = self.deps.require_local::<SnapshotEnc>(rvalue_ty).unwrap().generic_snapshot
                            .specifics
                            .expect_primitive();
    ...
    fn encode_place_element(
        ...
        match elem {
            mir::ProjectionElem::Deref => {
                assert!(place_ty.variant_index.is_none());
                let e_ty = self.deps.require_local::<SnapshotEnc>(place_ty.ty).unwrap().generic_snapshot;
                let place_ref = e_ty
                    .specifics
                    .expect_structlike()
                    .field_access
                ...
                match place_ty.ty.kind() {
                    TyKind::Closure(_def_id, args) => {
                        let upvars = args
                            ...
                            .collect::<Vec<_>>()
                            .len();
                        let tuple_ref = self.deps.require_local::<ViperTupleEnc>(upvars).unwrap();
                        (tuple_ref.mk_elem(self.vcx, expr, field_idx), place_ref)
                    }
                    _ => {
                        let e_ty = self.deps.require_ref::<PredicateEnc>(place_ty.ty).unwrap();
                        let struct_like = e_ty.generic_predicate.expect_variant_opt(place_ty.variant_index);
    ...
    fn encode_prusti_builtin(
        ...
        match builtin.expect("call to unknown non-pure function in pure code") {
            PrustiBuiltin::SnapshotEquality => {
                ...
                let bool_cons = self.deps.require_local::<SnapshotEnc>(self.vcx.tcx.types.bool).unwrap().generic_snapshot
                    .specifics
                    .expect_primitive()
                    .prim_to_snap;
            ...
            PrustiBuiltin::Forall => {
                ...
                reify_args
                    .push(unsafe { std::mem::transmute(self.encode_operand(&curr_ver, &args[1])) });
                reify_args.extend(
                    qvars
                        .iter()
                ...
                let bool = self.deps.require_local::<SnapshotEnc>(self.vcx.tcx.types.bool).unwrap().generic_snapshot
                    .specifics;
