        ...
        match rvalue {
            ...
            mir::Rvalue::Ref(_, kind, place) => {
                let rvalue_snapshot_encoding = self
                    ...
                    .unwrap();
                let ty_arg_exprs = rvalue_snapshot_encoding.ty_arg_exprs(self.vcx);
                let snapshot = rvalue_snapshot_encoding.generic_snapshot;
                let e_rvalue_ty = snapshot.specifics.expect_structlike().field_snaps_to_snap;
            ...
            rv @ mir::Rvalue::BinaryOp(op, box (l, r)) => {
                ...
                let task = BinOp(rvalue_ty, *op, l_ty, r_ty);
            ...
            mir::Rvalue::Aggregate(box kind, fields) => match kind {
                ...
                mir::AggregateKind::Adt(..) | mir::AggregateKind::Tuple => {
                    ...
                    sl.snap_data
                        .field_snaps_to_snap
                        .apply(self.vcx, ty_args, casted_args)
                }
            ...
            mir::ProjectionElem::Field(field_idx, ty) => {
                match place_ty.ty.kind() {
                    TyKind::Closure(_def_id, args) => {
                        ...
                        (
                            tuple_ref.mk_elem(self.vcx, expr, field_idx.as_usize()),
                            place_ref,
                        )
                    }
                    tykind => {
