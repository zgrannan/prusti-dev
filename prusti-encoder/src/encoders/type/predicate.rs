        ...
        match task_key.kind() {
            ...
            TyKind::Tuple(tys) => {
                ...
                let fields: Vec<_> = tys
                    ...
                    .collect();
                let fields = enc.mk_field_apps(specifics.ref_to_field_refs, tys);
                let fn_snap_body =
...
impl<'vir, 'tcx> PredicateEncValues<'vir, 'tcx> {
    ...
    pub fn mk_param(
        ...
        self.predicates.push(
            self.vcx
                .mk_predicate(self.ref_to_pred.name(), self.self_decl, None),
        );
    ...
    fn finalize(
        ...
        let self_pred_app =
            self.vcx
                .mk_predicate_app_expr(self.ref_to_pred.apply(self.vcx, [self.self_ex], None));
        ...
        let upcast_ty_params: Vec<_> = self
            .generics
            .iter()
            .map(|p| self.vcx.mk_local_decl(p, &TYP_DOMAIN))
            .collect();
