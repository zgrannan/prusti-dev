...
impl MirBuiltinEnc {
    ...
    fn handle_bin_op<'vir, 'tcx>(
        ...
        let (pres, val) = match op {
            ...
            Add | Sub | Mul | Shl | Shr => (
                Vec::new(),
                Self::get_wrapped_val(vcx, val, prim_res_ty.prim_type, res_ty),
            ),
    ...
    fn handle_checked_bin_op<'vir, 'tcx>(
        ...
        // Wrapped value
        let wrapped_val_exp =
            Self::get_wrapped_val(vcx, val, e_rvalue_pure_ty.prim_type, rvalue_pure_ty);
        ...
        let tuple = e_res_ty
            ...
            .expect_structlike()
            .field_snaps_to_snap
            .apply(vcx, &[wrapped_val_snap, overflowed_snap]);
