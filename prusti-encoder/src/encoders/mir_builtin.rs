...
impl MirBuiltinEnc {
    fn handle_un_op<'vir, 'tcx>(
        ...
        let e_ty = deps.require_local::<SnapshotEnc>(ty).unwrap().generic_snapshot;
    ...
    fn handle_bin_op<'vir, 'tcx>(
        ...
        use mir::BinOp::*;
        let e_l_ty = deps.require_local::<SnapshotEnc>(l_ty).unwrap().generic_snapshot;
        let e_r_ty = deps.require_local::<SnapshotEnc>(r_ty).unwrap().generic_snapshot;
        let e_res_ty = deps.require_local::<SnapshotEnc>(res_ty).unwrap().generic_snapshot;
    ...
    fn handle_checked_bin_op<'vir, 'tcx>(
        ...
        let e_rvalue_pure_ty = deps.require_local::<SnapshotEnc>(rvalue_pure_ty).unwrap().generic_snapshot;
        let e_rvalue_pure_ty = e_rvalue_pure_ty.specifics.expect_primitive();
        let e_bool = deps.require_local::<SnapshotEnc>(bool_ty).unwrap().generic_snapshot;
